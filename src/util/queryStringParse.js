 ...
 * fields   => fields(fields, allowedFields, restrictedFields)
 * filter   => filter(obj)
 * skip     => skip(val)
 * limit    => limit(val)
 * sort     => sort(fields)
...
module.exports = {
  /**
   * Convert select fields to mongodb project
   *
   * @param {String} fields
  ...
  fields (fields = '', allowedFields = [], restrictedFields = []) {
    ...
    for (let i = 0; i < arrayOfFields.length; i++) {
      if (allowedFields.includes(arrayOfFields[i].trim())) {
  ...
  // for example 'field1,+field2,-field3' -> {field1:1,field2:1,field3:-1}
  sort (fields) {
